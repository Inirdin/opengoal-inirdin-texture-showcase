...
bool MercLoaderStage::run(Timer& /*timer*/, LoaderInput& data) {
  ...
  if (!m_opengl) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 ...
                 nullptr, GL_STATIC_DRAW);
    m_opengl = true;
    auto& merc_data = data.lev_data->level->merc_data;
    for (auto& model : merc_data.models) {
      if (model.name == "ogreboss-village2-lod0") {
        for (auto& effect : model.effects) {
          for (auto& draw : effect.draws) {
            for (u32 idx = 0; idx < draw.index_count; idx++) {
              u32 vidx = merc_data.indices.at(idx + draw.first_index);
              if (vidx != UINT32_MAX) {
                auto& vtx = merc_data.vertices.at(vidx);
                // Brown parts, metal and fabric - MAX & MIN values
                if (vtx.rgba[0] < 125 & vtx.rgba[0] > 25
                  & vtx.rgba[1] < 70 & vtx.rgba[1] > 16
                  & vtx.rgba[2] < 58 & vtx.rgba[2] > 11) {
                  vtx.rgba[0] = 46;
                  vtx.rgba[1] = 35;
                  vtx.rgba[2] = 36;
                }
                // Red parts, boots and shorts
                else if (vtx.rgba[0] > 10 & vtx.rgba[1] < 18 & vtx.rgba[2] < 18) {
                  vtx.rgba[0] = 25;
                  vtx.rgba[1] = 23;
                  vtx.rgba[2] = 48;
                }
                // Blue parts, fur and legs - MAX & MIN values
                else if (vtx.rgba[0] < 100 & vtx.rgba[0] > 25
                  & vtx.rgba[1] < 100 & vtx.rgba[1] > 30
                  & vtx.rgba[2] < 110 & vtx.rgba[2] > 35) {
                  vtx.rgba[0] = 61;
                  vtx.rgba[1] = 62;
                  vtx.rgba[2] = 89;
                }
      ...
      if (model.name == "ogreboss-shoot-boulder-lod0" ||
          model.name == "ogreboss-shoot-boulder-break-lod0" ||
          model.name == "ogreboss-super-boulder-lod0" ||
          model.name == "ogreboss-bounce-boulder-lod0") {
        for (auto& effect : model.effects) {
          for (auto& draw : effect.draws) {
            for (u32 idx = 0; idx < draw.index_count; idx++) {
              u32 vidx = merc_data.indices.at(idx + draw.first_index);
              if (vidx != UINT32_MAX) {
                auto& vtx = merc_data.vertices.at(vidx);
                // Brown parts, metal and fabric - MAX & MIN values
                if (vtx.rgba[0] < 255 & vtx.rgba[0] > 0
                  & vtx.rgba[1] < 128 & vtx.rgba[1] > 0
                  & vtx.rgba[2] < 100 & vtx.rgba[2] > 0) {
                  vtx.rgba[0] = 100;
                  vtx.rgba[1] = 110;
                  vtx.rgba[2] = 128;
                }
